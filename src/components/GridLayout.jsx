...
import { dashboardReducer, initDashboardState } from "../store/dashboardStore";
...
const GridLayout = ({chartState}) => {
  ...
  const [dashboardState, dispatch] = useReducer(dashboardReducer, initDashboardState)

//   console.log("title is here:", title)
  const [layouts, setLayouts] = useState(null);
  const [widgetArray, setWidgetArray] = useState([
    { i: uuidv4(), x: 0, y: 0, w: 5, h: 1.5 },
  ]);
            ...
            return (
                ...
                >  
...

                  {
                    chartState.type !== ""
                    ? <>
                        <Box component="div" sx={{width: "100%", height:"100%", marginTop:"4rem"}} p={3}>
                        <GraphTypeSwitcher type={`${chartState.type}`} data={data} width={500} height={300}/>
                        </Box>
                      </>
                    :  <>
                    <Button
                      component={Link} to={`/${title}/edit`}
                      sx={{width:"100%", height:"100%", }}

                    >
                      <Typography variant="h5" sx={{marginTop: "1rem"}}>Add a new panel</Typography>
